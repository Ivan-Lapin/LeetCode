...
| 2220 | [Minimum Bit Flips to Convert Number](https://leetcode.com/problems/minimum-bit-flips-to-convert-number/description/) | Return the number of differing bits between start and goal by counting 1s in (start XOR goal). | Easy |
| 870 | [Advantage Shuffle](https://leetcode.com/problems/advantage-shuffle/description/) | Rearrange A to maximize the count of indices i where A[i] > B[i] (greedy: sort A, use two-pointer against sorted B). | Medium |
| 1920 | [Build Array from Permutation](https://leetcode.com/problems/build-array-from-permutation/description/) | Construct array ans where ans[i] = nums[nums[i]] given nums is a 0-indexed permutation of 0..n-1. | Easy |
| 2451 | [Odd String Difference](https://leetcode.com/problems/odd-string-difference/description/) | Given equal-length words, compute adjacent letter-difference arrays; return the unique word whose difference pattern differs from the rest. | Easy |
| 520 | [Detect Capital](https://leetcode.com/problems/detect-capital/description/) | Return true if a word uses capitals correctly: all letters uppercase, all lowercase, or only first letter uppercase (proper case). | Easy |
| 2085 | [Count Common Words With One Occurrence](https://leetcode.com/problems/count-common-words-with-one-occurrence/description/) | Count words that appear exactly once in each of two arrays; use frequency maps and sum words with countA[w] = 1 and countB[w] = 1. | Easy |
| 1711 | [Count Good Meals](https://leetcode.com/problems/count-good-meals/) | Count pairs of dishes whose sum is a power of two; for each value v, add counts of (2^k - v) seen so far, iterating k over powers up to max sum, with modulo 1e9+7. | Medium |
| 2535 | [Difference Between Element Sum and Digit Sum of an Array](https://leetcode.com/problems/difference-between-element-sum-and-digit-sum-of-an-array/description/) | Compute the absolute difference between the sum of elements and the sum of all digits across the array; sum(nums) vs. sum of digits of each num. | Easy |
| 2733 | [Neither Minimum nor Maximum](https://leetcode.com/problems/neither-minimum-nor-maximum/description/) | Return any element that is neither the minimum nor the maximum in an array of distinct positives; if none exists (length < 3), return -1. ​ | Easy ​ |
| 881 | [Boats to Save People]([https://leetcode.com/problems/boats-to-save-people/description/]) | Minimum boats to carry all people when each boat holds at most two and total weight ≤ limit; greedy two-pointer after sorting: pair heaviest with lightest if possible, otherwise send heaviest alone. ​ | Medium ​ |
| 2544 | [Alternating Digit Sum](https://leetcode.com/problems/alternating-digit-sum/description/) | Assign a positive sign to the most significant digit and alternate signs thereafter; return the signed sum of digits, computed in O(log n) time and O(1) space. ​ | Easy ​ |
| 1493 | [Longest Subarray of 1's After Deleting One Element](https://leetcode.com/problems/longest-subarray-of-1s-after-deleting-one-element/description/) | Sliding window with at most one zero in the window; track window length and subtract 1 (for the deleted element) to get the longest all-1s subarray. | Medium |
| 637 | [Average of Levels in Binary Tree](https://leetcode.com/problems/average-of-levels-in-binary-tree/description/) | Return the average of node values at each tree level using BFS (level-order): for each level, sum values and divide by node count. ​ | Easy ​ |
| 3222 | [Find the Winning Player in Coin Game](https://leetcode.com/problems/find-the-winning-player-in-coin-game/description/) | Each valid move must take coins totaling 115, which is only possible as 1×75 + 4×10; the number of full moves is t = min(x, ⌊y/4⌋), so Alice wins iff t is odd, else Bob. ​ | Easy ​ |
| 2401 | [Longest Nice Subarray](https://leetcode.com/problems/longest-nice-subarray/description/) | Longest subarray where every pair has bitwise AND 0; use sliding window with a running bitmask: expand right, and while (mask & nums[r]) ≠ 0, shrink left by clearing bits (mask ^= nums[l]). Answer is max window length. ​ | Medium ​ |
| 1672 | [Richest Customer Wealth](https://leetcode.com/problems/richest-customer-wealth/description/) | For each customer (row), sum balances across banks and return the maximum row sum; runs in $$O(mn)$$ time and $$O(1)$$ extra space. | Easy |
| 2614 | [Prime In Diagonal](https://leetcode.com/problems/prime-in-diagonal/description/) | Return the largest prime appearing on either main diagonal of the square matrix; scan both diagonals and track the maximum prime using an $$O(\sqrt{v})$$ primality check. | Easy |
| 1047 | [Remove All Adjacent Duplicates In String](https://leetcode.com/problems/remove-all-adjacent-duplicates-in-string/description/) | Repeatedly remove adjacent equal pairs; simulate with a stack/StringBuilder: push char if different from top, else pop, yielding the final string in $$O(n)$$ time and $$O(n)$$ space. | Easy |
| 413 | [Arithmetic Slices](https://leetcode.com/problems/arithmetic-slices/description/) | Count contiguous subarrays of length ≥ 3 with constant difference; scan once and accumulate streaks: if nums[i]−nums[i−1] equals previous diff, add current streak to answer, else reset. $$O(n)$$ time, $$O(1)$$ space. | Medium |
| 1046 | [Last Stone Weight](https://leetcode.com/problems/last-stone-weight/description/) | Use a max-heap to repeatedly smash the two heaviest stones; if unequal, push the difference back. Continue until ≤1 stone remains; return that weight or 0. $$O(n \log n)$$. | Easy |
| 3115 | [Maximum Prime Difference](https://leetcode.com/problems/maximum-prime-difference/description/) | Find the leftmost and rightmost indices containing primes in nums and return their difference; check primality up to 100 via sieve or trial division. | Easy |
| 435 | [Non-overlapping Intervals](https://leetcode.com/problems/non-overlapping-intervals/description/) | Remove the minimum number of intervals to eliminate overlaps; greedy by sorting by end time and keeping intervals with earliest finish to maximize non-overlapping count, answer = n − kept. $$O(n \log n)$$. | Medium |
| 504 | [Base 7](https://leetcode.com/problems/base-7/description/) | Convert integer to base-7 string: handle 0, record sign, repeatedly divide by 7 collecting remainders, then reverse; negative numbers get a leading minus. $$O(\log_7 n)$$ time, $$O(1)$$ extra space. | Easy |
| 1052 | [Grumpy Bookstore Owner](https://leetcode.com/problems/grumpy-bookstore-owner/description/) | Maximize satisfied customers by adding a sliding window of size X over grumpy==1 minutes: baseline = sum where grumpy==0, bonus = max over windows of sum(customers[i] where grumpy[i]==1). Answer = baseline + max bonus. $$O(n)$$. | Medium |
| 3038 | [Maximum Number of Operations With the Same Score I](https://leetcode.com/problems/maximum-number-of-operations-with-the-same-score-i/description/) | Let target = nums + nums[1], then count consecutive front pairs with the same sum: for i from 0 stepping by 2, stop when i+1 out of range or nums[i]+nums[i+1] ≠ target. $$O(n)$$ time, $$O(1)$$ space. | Easy |
| 3483 | [Unique 3-Digit Even Numbers](https://leetcode.com/problems/unique-3-digit-even-numbers/description/) | Count distinct three-digit even numbers formable from given digits without leading zeros and using each digit at most as many times as it appears; try all positions with a frequency map or 3 nested loops and store in a set. | Easy |
| 2785 | [Sort Vowels in a String](https://leetcode.com/problems/sort-vowels-in-a-string/description/) | Keep consonants fixed and sort only vowels by ASCII; extract vowels, sort them, then rebuild string replacing vowel positions in order. $$O(n \log n)$$ time, $$O(n)$$ space. | Medium |
| 2138 | [Divide a String Into Groups of Size k](https://leetcode.com/problems/divide-a-string-into-groups-of-size-k/description/) | Split s into consecutive substrings of length k; pad the final chunk with the fill character if shorter than k. Straight slice in steps of k; $$O(n)$$ time, $$O(n)$$ space. | Easy |
| 876 | [Middle of the Linked List](https://leetcode.com/problems/middle-of-the-linked-list/description/) | Return the middle node using two pointers: move slow by 1 and fast by 2; when fast reaches the end, slow is at the middle (for even length, this yields the second middle). $$O(n)$$ time, $$O(1)$$ space. | Easy |
| 3163 | [String Compression III](https://leetcode.com/problems/string-compression-iii/description/) | Compress runs of the same char but cap each run at 9: iterate, count up to 9 for each contiguous block, append count and char to output, then continue from the remainder. Example: "aaaaaaaaaaaaaabb" → "9a5a2b". $$O(n)$$ time, $$O(n)$$ space. | Medium |
| 1004 | [Max Consecutive Ones III](https://leetcode.com/problems/max-consecutive-ones-iii/description/) | Find the longest subarray of 1s by flipping at most k zeros: use sliding window, expand right while flips ≤ k, shrink left when flips > k, answer is max window size. $$O(n)$$ time, $$O(1)$$ space. | Medium |
| 2460 | [Apply Operations to an Array](https://leetcode.com/problems/apply-operations-to-an-array/description/) | For each i, if nums[i] == nums[i+1], set nums[i] = 2·nums[i] and nums[i+1] = 0; after one left-to-right pass, stable-compact nonzeros to the front and fill the rest with zeros. $$O(n)$$ time, $$O(1)$$ extra space. | Easy |
| 1189 | [Maximum Number of Balloons](https://leetcode.com/problems/maximum-number-of-balloons/description/) | Count frequencies of the letters in "balloon": need 1×b, 1×a, 2×l, 2×o, 1×n; answer is min(count[b], count[a], count[l]//2, count[o]//2, count[n]). $$O(n)$$ time, $$O(1)$$ space. | Easy |
...
| [0105-construct-binary-tree-from-preorder-and-inorder-traversal](https://github.com/Ivan-Lapin/LeetCode/tree/master/0105-construct-binary-tree-from-preorder-and-inorder-traversal) |
...
| [0075-sort-colors](https://github.com/Ivan-Lapin/LeetCode/tree/master/0075-sort-colors) |
| [0086-partition-list](https://github.com/Ivan-Lapin/LeetCode/tree/master/0086-partition-list) |
| [0141-linked-list-cycle](https://github.com/Ivan-Lapin/LeetCode/tree/master/0141-linked-list-cycle) |
| [0142-linked-list-cycle-ii](https://github.com/Ivan-Lapin/LeetCode/tree/master/0142-linked-list-cycle-ii) |
...
|  |
| ------- |
| [0190-reverse-bits](https://github.com/Ivan-Lapin/LeetCode/tree/master/0190-reverse-bits) |
...
| ------- |
| [0143-reorder-list](https://github.com/Ivan-Lapin/LeetCode/tree/master/0143-reorder-list) |
| [0155-min-stack](https://github.com/Ivan-Lapin/LeetCode/tree/master/0155-min-stack) |
...
| [0050-powx-n](https://github.com/Ivan-Lapin/LeetCode/tree/master/0050-powx-n) |
| [0143-reorder-list](https://github.com/Ivan-Lapin/LeetCode/tree/master/0143-reorder-list) |
| [0203-remove-linked-list-elements](https://github.com/Ivan-Lapin/LeetCode/tree/master/0203-remove-linked-list-elements) |
| [0206-reverse-linked-list](https://github.com/Ivan-Lapin/LeetCode/tree/master/0206-reverse-linked-list) |
| [0234-palindrome-linked-list](https://github.com/Ivan-Lapin/LeetCode/tree/master/0234-palindrome-linked-list) |
| [0394-decode-string](https://github.com/Ivan-Lapin/LeetCode/tree/master/0394-decode-string) |
| [3799-unique-3-digit-even-numbers](https://github.com/Ivan-Lapin/LeetCode/tree/master/3799-unique-3-digit-even-numbers) |
## Enumeration
